...
def example_plot_1d():
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(
        3, 1, gridspec_kw={"height_ratios": [1, 3, 1]}, figsize=(10, 8)
    )
    ...
    ax1.plot(
        ...
        ls="solid",
        c="orange",
        label="highest observed value",
        zorder=5,
    )
    ax1.plot(
        [2, 12], [max(y_func), max(y_func)], ls="dashed", label="actual max", c="black"
    )
    ...
    ax2.plot(
        proposal,
        search_function(proposal),
        "o",
        c="green",
        label="proposed observation",
    )
    ax3.set_ylim([0, 1])
    ax3.set_yticks([])
    ax3.set_xlabel("x")
